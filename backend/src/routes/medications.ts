...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (genericName) medication.genericName = genericName;
    if (brandName !== undefined) medication.brandName = brandName;
    if (category) medication.category = category;
    if (description) medication.description = description;
    if (indications !== undefined) medication.indications = indications;
    if (contraindications !== undefined) medication.contraindications = contraindications;
    if (sideEffects !== undefined) medication.sideEffects = sideEffects;
    if (interactions !== undefined) medication.interactions = interactions;
    if (warnings !== undefined) medication.warnings = warnings;
    if (dosageInfo !== undefined) medication.dosageInfo = dosageInfo;
...
router.get('/prescriptions/all', async (req: Request, res: Response) => {
  try {
    const user = (req as any).user;
