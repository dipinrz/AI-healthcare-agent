...
export class AvailabilitySeedService {
  ...
  async generateMonthlySlots(doctorId?: string) {
    try {
      let doctors: Doctor[] = [];
  ...
  private generateDailySlots(doctor: Doctor, date: Date): DoctorAvailability[] {
    const slots: DoctorAvailability[] = [];
  ...
  private async createDemoDoctors() {
    const demoDoctors = [
      {
        firstName: 'John',
        lastName: 'Smith',
