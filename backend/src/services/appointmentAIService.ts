...
interface ExtractedInfo {
  intent: 'book_appointment' | 'check_availability' | 'cancel_appointment' | 'reschedule_appointment' | 'general_query';
  doctorName?: string;
  doctorId?: string;
  date?: string;
  time?: string;
...
interface ConversationContext {
  currentFlow?: 'booking' | 'checking_availability' | 'confirming_booking' | 'canceling_appointment' | 'rescheduling_appointment';
...
export class AppointmentAIService {
  ...
  ): Promise<{ response: string; context: ConversationContext; actions?: any[] }> {
  ...
  private extractIntentAndEntities(message: string): ExtractedInfo {
    const lowerMessage = message.toLowerCase();
    
    // Intent detection
    let intent: ExtractedInfo['intent'] = 'general_query';
  ...
  private containsBookingKeywords(message: string): boolean {
  ...
  private containsAvailabilityKeywords(message: string): boolean {
  ...
  private containsCancelKeywords(message: string): boolean {
  ...
  private containsRescheduleKeywords(message: string): boolean {
  ...
  private extractDoctorName(message: string): string | undefined {
  ...
  private extractAppointmentType(message: string): AppointmentType | undefined {
  ...
  private async findDoctorsByName(name: string): Promise<Doctor[]> {
    ...
    searchTerms.forEach((term, index) => {
      ...
      if (index === 0) {
        query = query.where(`(doctor.firstName ILIKE :${paramKey} OR doctor.lastName ILIKE :${paramKey})`, 
          { [paramKey]: `%${term}%` });
      } else {
        query = query.orWhere(`(doctor.firstName ILIKE :${paramKey} OR doctor.lastName ILIKE :${paramKey})`, 
          { [paramKey]: `%${term}%` });
      }
    });
  ...
  private async proceedWithDateTimeSelection(
    patientId: string, 
    context: ConversationContext, 
    doctor: Doctor
  ): Promise<{ response: string; context: ConversationContext; actions?: any[] }> {
  ...
  private async checkAvailabilityAndBook(
    patientId: string, 
    context: ConversationContext
  ): Promise<{ response: string; context: ConversationContext; actions?: any[] }> {
    ...
    try {
      ...
      const appointment = this.appointmentRepository.create({
        ...
        duration: 30,
        type: extractedInfo.appointmentType || AppointmentType.CONSULTATION,
  ...
  private async handleCancellation(
    ...
    return {
      response: `Here are your upcoming appointments:\n${appointments.map((apt, i) => 
        `${i + 1}. Dr. ${apt.doctor.firstName} ${apt.doctor.lastName} - ${this.formatDateTime(apt.appointmentDate)}`
  ...
  // Utility methods
  private async getAvailableSlots(doctorId: string, date: string): Promise<any[]> {
  ...
  private async isSlotAvailable(doctorId: string, appointmentDateTime: Date): Promise<boolean> {
