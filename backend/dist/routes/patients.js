...
router.get('/', async (req, res) => {
    try {
        ...
        if (search) {
            queryBuilder = queryBuilder.where('(patient.firstName ILIKE :search OR patient.lastName ILIKE :search OR patient.email ILIKE :search)', { search: `%${search}%` });
        }
        if (gender) {
        ...
        if (ageMin || ageMax) {
            ...
            if (ageMax) {
                const minBirthDate = new Date(currentDate.getFullYear() - parseInt(ageMax), 0, 1);
                queryBuilder = queryBuilder.andWhere('patient.dateOfBirth >= :minBirthDate', { minBirthDate });
            }
            if (ageMin) {
                const maxBirthDate = new Date(currentDate.getFullYear() - parseInt(ageMin), 11, 31);
...
router.post('/', async (req, res) => {
    try {
        ...
        const { firstName, lastName, email, phone, password, dateOfBirth, gender, address, allergies, emergencyContact } = req.body;
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const { firstName, lastName, phone, dateOfBirth, gender, address, allergies, emergencyContact, isActive } = req.body;
        if (firstName)
