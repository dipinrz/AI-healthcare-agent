...
const doctorAvailability_service_1 = require("./doctorAvailability.service");
...
class SeedService {
    constructor() {
        ...
        this.availabilityService = new doctorAvailability_service_1.DoctorAvailabilityService();
    ...
    async seedAllData() {
        try {
            ...
            logger_config_1.logger.info('📈 Seeding health records...');
            const healthRecordResult = await this.seedHealthRecords(25);
            results.healthRecords = healthRecordResult.created;
            logger_config_1.logger.info('✅ Master seed completed successfully!', results);
            return {
            ...
            logger_config_1.logger.error('❌ Master seed failed:', error);
            throw error;
        }
    }
    async seedDoctors(count = 10) {
        try {
            const doctors = [];
                ...
                if (!existingPatient) {
                    ...
                    try {
                        await this.authService.register({
                            email,
                            password: 'password123',
                            role: User_model_1.UserRole.PATIENT,
                            firstName,
                            lastName,
                            phone: `+1${Math.floor(Math.random() * 9000000000) + 1000000000}`,
                            dateOfBirth,
    ...
    async seedDoctorAvailability(doctorId, days = 30) {
        try {
            if (doctorId) {
    ...
    async seedAppointments(count = 50) {
        try {
    ...
    async seedPrescriptions(count = 30) {
        try {
            const doctors = await this.doctorRepository.findAvailableDoctors();
            const patients = await this.patientRepository.findActivePatients();
    ...
    async seedHealthRecords(count = 40) {
        try {
    ...
    async clearAllData() {
        try {
            logger_config_1.logger.warn('🗑️ CLEARING ALL DATA - This cannot be undone!');
            const results = {
                prescriptions: 0,
                appointments: 0,
                availability: 0,
            ...
            logger_config_1.logger.warn('🗑️ All data cleared successfully', results);
            return {
    ...
    async seedDemoScenario(scenario = 'basic') {
        try {
            switch (scenario) {
                case 'basic':
    ...
    async resetEntity(entity) {
        try {
            ...
            logger_config_1.logger.error(`Reset ${entity} error:`, error);
            throw error;
        }
    }
