...
class MedicationService {
    ...
    async getAllMedications(filters, page = 1, limit = 10) {
        try {
            logger_config_1.logger.info('Getting all medications with filters:', filters);
            let medications;
            if (filters.search) {
    ...
    async createMedication(medicationData) {
        try {
            ...
            logger_config_1.logger.info('Medication created successfully:', medication.id);
            return medication;
        }
        catch (error) {
    ...
    async updateMedication(id, updateData) {
        try {
            ...
            logger_config_1.logger.info('Medication updated successfully:', id);
    ...
    async deleteMedication(id) {
        try {
            ...
            logger_config_1.logger.info('Medication deleted successfully:', id);
        }
        catch (error) {
    ...
    async getMedicationsByCategory(category) {
        try {
            ...
            logger_config_1.logger.error('Get medications by category error:', error);
            throw error;
        }
    }
    async getMedicationsByDosageForm(dosageForm) {
        try {
            ...
            logger_config_1.logger.error('Get medications by dosage form error:', error);
            throw error;
        }
    }
    async getMedicationStats() {
        try {
            ...
            logger_config_1.logger.error('Get medication stats error:', error);
            throw error;
        }
    }
    async getPopularMedications(limit = 10) {
        try {
            ...
            logger_config_1.logger.error('Get popular medications error:', error);
            throw error;
        }
    }
}
